...
detect_os () {
    ...
    if [ "${REPO_OS}" = 'centos' ] || [ "${REPO_OS}" = 'rhel' ] || [ "${REPO_OS}" = 'rocky' ] || [ "${REPO_OS}" = 'almalinux' ] || [ "${REPO_OS}" = 'ol' ] || [ "${REPO_OS}" = 'redhat' ] || [ "${REPO_OS}" = 'virtuozzo' ] || [ "${REPO_OS}" = 'cloudlinux' ] ; then
        REPO_BASE_OS='centos'
...
install_thirdparty_repo () {
    if [ "${REPO_BASE_OS}" = 'centos' ] ; then
        echo 'Install third party packages repos.'
        if [ "${REPO_OS_VER}" = '9' ]; then
            dnf install https://rpms.remirepo.net/enterprise/remi-release-${REPO_OS_VER}.rpm -y
